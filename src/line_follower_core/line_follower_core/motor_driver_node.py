...
class MotorDriverNode(Node):
    def __init__(self):
        ...
        # Publishers (Always initialized for GUI monitoring)
        self.pub_l = self.create_publisher(Int32, "motor/left/pwm", 10)
        self.pub_r = self.create_publisher(Int32, "motor/right/pwm", 10)
        
        if self.hardware_mode:
            self.get_logger().info("Motor Driver: HARDWARE MODE (Controlling Real Motors)")
        else:
            self.get_logger().info("Motor Driver: SIMULATION MODE (Virtual PWM for Monitoring)")
